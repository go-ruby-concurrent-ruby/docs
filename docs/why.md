...
`go-ruby-concurrent-ruby/concurrent-ruby` reimplements Ruby's `concurrent-ruby` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-concurrent-ruby/concurrent-ruby` directly, with no Ruby runtime;
...
Because `concurrent-ruby` is pure Go with cgo disabled, it also compiles to **WebAssembly**
